...
class UserBookController extends Controller
{
    public function index(User $user, Request $r)
    {
        ...
        $sort = collect(explode(',', (string)$r->query('sort','title')))
            ->filter()->map(fn($f)=>[ltrim($f,'-'), str_starts_with($f,'-')?'desc':'asc']);
        foreach ($sort as [$field,$dir]) {
            if (in_array($field,['title','author','publication_year','created_at'])) $q->orderBy($field,$dir);
        }
